...
%               The exact solution I got was y = -cos(t)/2
% This is obtained as follows:
% y' = y*tan(t) + sin(t); 
% integrating factor = e^-(int(tan(t)) 
% = e^ln(cos(t)) = cos(t) (Methods of substitution was involved)

%cos(t)*y' - cos(t)*tan(t)*y = cos(t)*sin(t)
%cos(t)*y' - sin(t)*y = cos(t)*sin(t)

%Integrating both sides and taking integration by substitution  gives):
% y = (1/2)*[(sin(t)^2)/cos(t)] + [C/cos(t)]
%Given initial condition y(0) = -1/2 --> C = -(1/2)
%y = [(1/2)*cos(t)] * [(sin(t)^2) -1)
%y = -cos(t)/2

%Constructing the inline function 
f = @(t,y) (y*tan(t)) + sin(t);
...
%t will span from 0 to pi
t1 = pi;

%Using the ode45 to obtain the solution
ode_soln = ode45(f, [t0, t1], y0);

exact_x = linspace(0,pi);
exact_soln = (-cos(exact_x)/2);

%Plotting the solutions
plot(exact_x, exact_soln, ode_soln.x, ode_soln.y, 'x', 'MarkerSize',10, 'LineWidth', 2);
...
%Using deval to compute interpolated values
interpolate_val = linspace(2, 3, 10);
deval(ode_soln, interpolate_val)
...
y_exact = (-cos(ode_soln.x)/2);

%Computing the pointwise error
error = abs(y_exact - ode_soln.y);

disp(error);

fprintf('maximum error: %g \n', max(error));

semilogy(ode_soln.x, error, 'LineWidth', 2);
xlabel('t');
ylabel('error');

%The error is largest at t = 1.5581 seconds because the solution has a
%point of inflection, indicating that its derivative is at a maximum.
...
%t will span from 1 to 10
t1 = 10;

%Using the ode45 to obtain the computation of the solution at grid points.
ode_soln2 = ode45(f, [t0, t1], y0);


%The exact solution is y = (3t - 2)^(1/3)

%Computing the exact solution. 
exact_soln2 = ((3.*ode_soln2.x)-2).^(1/3);

%Plotting the exact and approximate solutions.
plot(ode_soln2.x, exact_soln2, ode_soln2.x, ode_soln2.y, 'x', 'MarkerSize',10, 'LineWidth', 2);
xlabel('t');
ylabel('y');
legend("exact solution", "ode45");


%Computing the maximum pointwise error
error2 = abs(exact_soln2 - ode_soln2.y);
...
plot(ode_soln.x, ode_soln.y, "LineWidth", 2);
xlabel('t');
ylabel('y');
...
%Observations:
%1) Graph fails to integrate all the way to t = 1. Instead, it spikes
%upwards when t = 0.5
%2) The error message: 
%Warning: Failure at t=5.066046e-01.  Unable to meet integration tolerances without 
%reducing the step size below the smallest value allowed (1.776357e-15) at time t. 


%Explanation: 
%After graphing it with IODE, I noticed that the direction field goes
%towards infinity as it approaches t = 0.5. This indicates that the solution is only
%defined until t = 0.5, thus the ode45 can no longer compute the numerical solution after t = 0.5
...
%For x = pi/4
x_pi = subs(s,x,(pi/4));
fprintf('When x = pi/4, y = %g\n', x_pi);


%For x = 1
one = subs(s,x,1);
fprintf('When x = 1, y = %g\n', one);
...
% Setting up our initial approximation as phi_5 = 1:
phi_5=[sym(1)];  

% Setting up a loop to get successive approximations using Picard iterations
N=5;

for i = 1:N
    func=subs(g,y,phi_5(i));   
    func=subs(func,t,s);     
    newphi =1 + int(func, s, 0 ,t);   %The initial conditions y(0) = 1 
    phi_5=cat(2,phi_5,[newphi]);      
end

% Showing the last approximation
phi_5(N+1)

% Plotting the approximation just found and adjusting the range
figure; 
picard2=ezplot(phi_5(N+1),[-5, 5]);
set(picard2,'Color','green', "LineWidth", 1); 
hold on;

%           The exact solution I calculated is: y = tan(t)
exact2=ezplot(tan(t), [-5, 5]);
set(exact2,'Color','red', "LineWidth", 2, "LineStyle", "--"); 
